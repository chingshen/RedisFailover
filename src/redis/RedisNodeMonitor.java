 ...
 * e.g. src/redis-server ./redis1.conf | ./redis2.conf and so on...
 * 
...
public class RedisNodeMonitor implements Watcher,  AsyncCallback.ChildrenCallback
{
	...
	public void startRedisMonitor()
	{
    	...
    	{
    		logger.info("Could not connect to Redis server: "+this.redisAddress+", "+e);
			e.printStackTrace();
			System.exit(-1);	
    	}catch(Exception e)
    	{
	...
	public void slaveOfMaster() 
	{
		try
		{
			...
			if(this.masterAddress != null && masterData != null && this.masterAddress.equals(masterData))
				return;
	...
	public void process(WatchedEvent event)
	{
		...
		logger.debug("~~~~~~~~~~~~~~~~~~~~~~~~~~START~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
		logger.debug("Got ZooKeeper event, state: " + state + ", type: " + type + ", path: " + path);
		logger.debug("~~~~~~~~~~~~~~~~~~~~~~~~~~~END~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
            ...
            case SyncConnected:
            	try
    			{
    				...
    				if (stat == null)
    				{
    					...
    					logger.debug("Connected to Redis master on "+ this.redisAddress +".");
    					promoteToMaster();
    				}else{
    					slaveOfMaster();
    				}
